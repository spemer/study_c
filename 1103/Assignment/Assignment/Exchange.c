...
float exchange_jpy(int KRW, float money) //엔화 환전
{
	float out = ((float)KRW * 100) / money;
	float actual_out = (floor(out / 10)) * 10; //10으로 나눠 floor를 한 값에 다시 10을 곱함(엔화)
	return actual_out;
}
int change_jpy(int KRW, float out, float money) //엔화 거스름돈
{
	float change = KRW - ((out / 100) * money);
	return ceil(change); //엔화일 경우의 거스름돈을 올림
}
char *country; //나라 영문 이름
int main(void)
{
	...
	{
		money = JPY; //상수 JPY 엔화를 money에 대입
	...
	else //일본 엔화가 아닌 경우
	{
		...
		{
		case 'u': case 'U':
			money = USD; //u 또는 U(미국달러)일 경우에 상수 USD 미국달러를 money에 대입
			country = "USD"; //금액 단위(USD)를 변수(country)에 대입
			break;
		case 'e': case 'E':
			money = EUR;
			country = "EUR";
